...
-- "_" - один символ, % - любое кол-во символов , \ - экранирование , т.е. \% - берем именно данный символ процента
-- [0-9] - любой символ из множества 0-9
-- {11} - только 11 символов может быть
...
CREATE TABLE message_status (
  ...
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания строки",  
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT "Время обновления строки"  
) COMMENT "Стасусы доставки сообщений";
...
-- ALTER TABLE media DROP COLUMN entity_type;
...
INSERT INTO `users` (`id`, `first_name`, `last_name`, `email`, `phone`, `created_at`, `updated_at`) VALUES (2, 'Roman', 'Towne', 'eric.paucek@example.net', '07789423106', '1982-09-08 17:18:50', '2018-01-28 13:30:53');
INSERT INTO `users` (`id`, `first_name`, `last_name`, `email`, `phone`, `created_at`, `updated_at`) VALUES (3, 'Irving', 'O\'Hara', 'ibashirian@example.net', '155.550.0675', '1982-06-27 03:30:46', '1993-01-09 16:47:02');
...
INSERT INTO `messages` (`from_user_id`, `to_user_id`, `status_id`, `body`, `is_important`, `is_delivered`) values (5,15,1,'Привет, Давай познакомимся?', 0,1);
INSERT INTO `messages` (`from_user_id`, `to_user_id`, `status_id`, `body`, `is_important`, `is_delivered`) values (18,43,1,'Привет, как дела', 0,1);
INSERT INTO `messages` (`from_user_id`, `to_user_id`, `status_id`, `body`, `is_important`, `is_delivered`) values (2,88,2,'Привет, я скучал', 0,1);
INSERT INTO `messages` (`from_user_id`, `to_user_id`, `status_id`, `body`, `is_important`, `is_delivered`) values (5,97,1,'Привет, почему ты меня игнорируешь?', 1,1);
INSERT INTO `messages` (`from_user_id`, `to_user_id`, `status_id`, `body`, `is_important`, `is_delivered`) values (5,3,2,'Привет, ты сделал домашку?', 1,0);
...
INSERT INTO `entity_types` (`id`, `name`,`created_at`, `updated_at`) values (1,'photo',current_timestamp(), current_timestamp());
INSERT INTO `entity_types` (`id`, `name`,`created_at`, `updated_at`) values (2,'video',current_timestamp(), current_timestamp());
INSERT INTO `entity_types` (`id`, `name`,`created_at`, `updated_at`) values (3,'file',current_timestamp(), current_timestamp());

INSERT INTO `message_statuses` (`id`, `name`) values (1,'доставдено');
INSERT INTO `message_statuses` (`id`, `name`) values (2,'прочитано');
...
ALTER TABLE messages ADD CONSTRAINT fk_entity_type_id FOREIGN KEY (entity_type_id) REFERENCES entity_types (id);


ALTER TABLE messages ADD COLUMN entity_type_id INT UNSIGNED;
...
ALTER TABLE likes_users ADD CONSTRAINT fk_like_user FOREIGN KEY (user_id) REFERENCES users (id);
...
-- ALTER TABLE media DROP CONSTRAINT fk_entity_types;
-- ALTER TABLE media DROP CONSTRAINT entity_type_id;

-- ALTER TABLE media ADD CONSTRAINT fk_entity_types FOREIGN KEY (entity_type_id) REFERENCES entity_types (id);
-- ALTER TABLE media DROP CONSTRAINT fk_entity_types;
